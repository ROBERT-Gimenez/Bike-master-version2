 ...
 */const toThousand = n => n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");4
const db = require('../../database/models')

let products = db.Producto.findAll().then((products)=>{products})
...
module.exports = {
    ...
    userAdmin:(req , res , next) =>{
        if(req.session.user){
            ...
            writeUsers(users); */
            let adminuser = req.session.user.rol = 2;
            db.Usuario.update({rol_id:2},
